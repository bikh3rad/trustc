...
import (
	...
	"github.com/trustc/trustc/services/governance/internal/api"
...
func main() {
	...
	db, err := pg.Connect(ctx, "governance")
	if err != nil {
	...
	addr := envOr("TRUSTC_GOVERNANCE_ADDR", ":7006")
	srv := &http.Server{Addr: addr, Handler: root, ReadHeaderTimeout: 5 * time.Second}
	log.Info("governance listening", "addr", addr)
